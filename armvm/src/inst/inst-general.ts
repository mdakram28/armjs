...
INST.mov = class Inst_mov extends Inst {
    run(vm: ArmVM): void {
        ...
        vm._state.set(this.tokens[1], val)
    }
}
...
INST.b = class Inst_b extends Inst {
    run(vm: ArmVM): boolean {
        ...
        if (addr) {
            vm._state.setPC(vm.nextNonNullInstAddr(vm.toRamAddr(addr)))
            return true
...
INST.bl = class Inst_bl extends Inst {
    run(vm: ArmVM): boolean {
        let label = this.tokens[1].value
        const addr = vm.findLabelAddr(label, this.tokens[1])
        if (vm._state.ram[vm.toRamAddr(addr)] instanceof Function) {
            // Library function, Execute immediately
            (vm._state.ram[vm.toRamAddr(addr)] as LibFunc)(vm)
            return false
        } else if (addr) {
            vm._state.setPC(vm.nextNonNullInstAddr(vm.toRamAddr(addr)))
            return true
...
INST.add = class Inst_add extends Inst {
    run(vm: ArmVM): void {
        let val = vm._state.get(this.tokens[2].value, this.tokens[2])
...
INST.cmp = class Inst_cmp extends Inst {
    run(vm: ArmVM): void {
        ...
        let val1 = vm._state.get(r1, this.tokens[1])
